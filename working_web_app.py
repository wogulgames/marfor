...
def get_time_series_data(session_id):
    ...
    try:
        print(f"🔧 ВЕРСИЯ КОДА: 2.8.0 - Переписан функционал кнопок заголовков как триггеры")
        if not session_id or forecast_app.session_id != session_id:
            return jsonify({'success': False, 'message': 'Сессия не найдена'})
...
if __name__ == '__main__':
    ...
    print("🔧 ВЕРСИЯ КОДА: 2.8.0 - Переписан функционал кнопок заголовков как триггеры")
