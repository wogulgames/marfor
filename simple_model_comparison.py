...
# -*- coding: utf-8 -*-
"""
Упрощенное сравнение моделей прогнозирования
Включает: Linear Regression, Ridge, Lasso, Random Forest, Prophet
"""
...
class SimpleModelComparison:
    ...
    def compare_models(self):
        ...
        print(f"\n  🥇 ОБЩИЙ РЕЙТИНГ МОДЕЛЕЙ (по среднему R²):")
        model_ranking = metrics_df.groupby('model')['r2'].mean().sort_values(ascending=False)
        for i, (model, r2) in enumerate(model_ranking.items(), 1):
            print(f"    {i}. {model}: R²={r2:.3f}")
        
        return metrics_df
    
    def save_comparison_results(self, output_file='Simple_Model_Comparison_Results.csv'):
...
def main():
    ...
    print("="*60)
    print("Модели: Linear Regression, Ridge, Lasso, Random Forest, Prophet")
    print("="*60)
    
    # Инициализация
    comparator = SimpleModelComparison('Marketing Budjet Emulation - raw2.csv')
