...
# -*- coding: utf-8 -*-
"""
Продвинутая программа прогнозирования с группировкой по регионам, подразделениям и категориям
"""
...
class AdvancedGroupedForecaster:
    def __init__(self, csv_file=None):
        """Инициализация продвинутого прогнозировщика с группировкой"""
    ...
    def clean_data(self):
        ...
        for col in self.df.columns:
            if col not in ['year', 'month', 'region_to', 'subdivision', 'category']:
    ...
    def analyze_groups(self):
        """Анализ групп данных"""
        print("\n" + "="*60)
        print("АНАЛИЗ ГРУПП ДАННЫХ")
        print("="*60)
        
        # Анализ регионов
        if 'region_to' in self.df.columns:
            print(f"\nРегионы:")
            region_counts = self.df['region_to'].value_counts()
        ...
        if 'subdivision' in self.df.columns:
            print(f"\nПодразделения:")
            subdivision_counts = self.df['subdivision'].value_counts()
            for subdivision, count in subdivision_counts.items():
                print(f"  {subdivision}: {count} записей")
        ...
        if all(col in self.df.columns for col in ['region_to', 'subdivision', 'category']):
            print(f"\nТоп-10 комбинаций region_to + subdivision + category:")
            combo_counts = self.df.groupby(['region_to', 'subdivision', 'category']).size().sort_values(ascending=False)
            for (region, subdivision, category), count in combo_counts.head(10).items():
                print(f"  {region} + {subdivision} + {category}: {count} записей")
    ...
    def auto_detect_columns(self):
        ...
        exclude_keywords = ['id', 'index', 'key', 'code', 'year', 'month']
        target_candidates = []
    ...
    def create_group_features(self, df):
        ...
        if 'region_to' in df.columns:
            ...
            df['region_encoded'] = le_region.fit_transform(df['region_to'].astype(str))
        
        if 'subdivision' in df.columns:
            le_subdivision = LabelEncoder()
            df['subdivision_encoded'] = le_subdivision.fit_transform(df['subdivision'].astype(str))
        
        if 'category' in df.columns:
            le_category = LabelEncoder()
            df['category_encoded'] = le_category.fit_transform(df['category'].astype(str))
        
        # Взаимодействие времени с группами
        if 'time_index' in df.columns and 'region_encoded' in df.columns:
            df['time_region'] = df['time_index'] * df['region_encoded']
        
        if 'time_index' in df.columns and 'subdivision_encoded' in df.columns:
            df['time_subdivision'] = df['time_index'] * df['subdivision_encoded']
    ...
    def prepare_features(self, df):
        ...
        group_features = ['region_encoded', 'subdivision_encoded', 'category_encoded']
        for feature in group_features:
            if feature in df.columns:
                base_features.append(feature)
        
        # Добавляем взаимодействия
        interaction_features = ['time_region', 'time_subdivision']
    ...
    def train_grouped_models(self, test_size=0.3, cv_folds=3):
        """Обучение моделей с группировкой"""
        print("\n" + "="*60)
        print("ОБУЧЕНИЕ МОДЕЛЕЙ С ГРУППИРОВКОЙ")
        print("="*60)
        ...
        for target in self.target_columns:
            ...
            if len(train_data) < 50:  # Минимум 50 записей
    ...
    def create_forecast(self, forecast_periods=4):
        ...
        if not self.models:
            raise ValueError("Модели не обучены. Сначала выполните train_grouped_models()")
        ...
        if all(col in self.df.columns for col in ['region_to', 'subdivision', 'category']):
            ...
            unique_combinations = pd.DataFrame({'region_to': ['Unknown'], 'subdivision': ['Unknown'], 'category': ['Unknown']})
    ...
    def save_forecast(self, output_file='Advanced_Grouped_Forecast_Results.csv'):
    ...
    def plot_forecast_analysis(self, target_columns=None, save_plot=True):
        ...
        if save_plot:
            plt.savefig('advanced_grouped_forecast_analysis.png', dpi=300, bbox_inches='tight')
            print("График сохранен как 'advanced_grouped_forecast_analysis.png'")
        
        plt.show()

def main():
    """Основная функция для демонстрации"""
    print("Продвинутая программа прогнозирования с группировкой")
    print("="*60)
    
    # Инициализация
    forecaster = AdvancedGroupedForecaster('Marketing Budjet Emulation - raw2.csv')
    ...
    # Анализ групп
    forecaster.analyze_groups()
    
    # Автоматическое определение колонок
    forecaster.auto_detect_columns()
    
    # Обучение моделей с группировкой
    forecaster.train_grouped_models()
