...
# -*- coding: utf-8 -*-
"""
Программа прогнозирования с правильной валидацией модели
Основана на принципах линейной регрессии из статьи:
...
class ValidatedForecaster:
    def __init__(self, csv_file=None):
        """Инициализация валидированного прогнозировщика"""
    ...
    def clean_data(self):
        ...
        if 'revenue_total' in self.df.columns:
            ...
            print(f"  Минимальная выручка за период: {period_stats['sum'].min():,.0f} ₽")
    ...
    def split_data_properly(self, train_end_year=2024, train_end_month=12, 
        ...
        # Исключаем данные с сентября 2025 - это прогноз
        validation_mask = (
            (self.df['year'] > test_start_year) | 
            ((self.df['year'] == test_start_year) & (self.df['month'] > test_end_month))
        )
        ...
        print(f"  Исключенные данные (прогноз): {len(self.validation_df)} записей")
        ...
        if 'revenue_total' in self.train_df.columns:
            ...
            excluded_revenue = self.validation_df['revenue_total'].sum()
            
            print(f"\n💰 АНАЛИЗ ВЫРУЧКИ ПО ВЫБОРКАМ:")
            print(f"  Выручка в обучающей выборке: {train_revenue:,.0f} ₽")
            print(f"  Выручка в тестовой выборке: {test_revenue:,.0f} ₽")
            print(f"  Исключенная выручка (прогноз): {excluded_revenue:,.0f} ₽")
            ...
            if len(self.validation_df) > 0:
                val_start = f"{self.validation_df['year'].min()}.{self.validation_df['month'].min():02d}"
                val_end = f"{self.validation_df['year'].max()}.{self.validation_df['month'].max():02d}"
                print(f"  Исключенные данные: {val_start} - {val_end} (прогноз)")
...
def main():
    ...
    print("🎯 ПРОГРАММА ПРОГНОЗИРОВАНИЯ С ПРАВИЛЬНОЙ ВАЛИДАЦИЕЙ")
    print("="*70)
    print("Основана на принципах линейной регрессии:")
    print("https://medium.com/analytics-vidhya/understanding-the-linear-regression-808c1f6941c0")
    print("="*70)
    
    # Инициализация
    forecaster = ValidatedForecaster('Marketing Budjet Emulation - raw2.csv')
    ...
    if is_validated:
        print(f"\n🎉 МОДЕЛЬ ГОТОВА ДЛЯ ПРОГНОЗИРОВАНИЯ!")
