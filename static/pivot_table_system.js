...
function renderPivotChart(chartData, config) {
    ...
    // Google Sheets подход: график ВСЕГДА заполняет доступное пространство
    // Ось Y должна показывать только диапазон данных с минимальными отступами
    
    // Отступы: 5% от диапазона ДАННЫХ (не от значений!)
    const padding = range * 0.05;
    
    // Ось Y начинается чуть ниже минимума и заканчивается чуть выше максимума
    const yMin = minValue - padding;
    const yMax = maxValue + padding;
    
    console.log('📊 Google Sheets подход - фокус на диапазоне данных');
    console.log('Диапазон данных:', range.toLocaleString('ru-RU'));
    console.log('Отступ (5% от диапазона):', padding.toLocaleString('ru-RU'));
    console.log('yMin:', yMin.toLocaleString('ru-RU'), '| yMax:', yMax.toLocaleString('ru-RU'));
