...
function renderPivotChart(chartData, config) {
    ...
    // Для больших чисел с высокой вариативностью используем минимальные отступы
    // Это позволит максимально заполнить график
    let paddingPercent = 0.05; // Всегда 5% для максимальной наглядности
    
    const padding = range * paddingPercent;
    
    // ВСЕГДА фокусируемся на диапазоне данных
    // Используем отступ только вверх, а снизу начинаем от минимума
    const yMin = minValue - (range * 0.02); // Минимальный отступ 2%
    const yMax = maxValue + padding; // Отступ 5% сверху
